...
class MetricsService:
    ...
    def record_failure(self, agent_name: str, provider: str, model: str, error: str, duration_ms: int = 0) -> LLMUsageEntry:
        return self.record_llm_usage(
            agent_name=agent_name,
            provider=provider,
            model=model,
    ...
    def summary(self) -> LLMUsageSummary:
        ...
        for entry in self.entries:
            ...
            if entry.status == "failed":
                agent_bucket["failed_calls"] += 1
            elif entry.status == "estimated" or entry.estimated:
                agent_bucket["estimated_calls"] += 1
            else:
                agent_bucket["success_calls"] += 1
